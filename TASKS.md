...
- **DONE (2026-06-03) — classifier keyword precision.** Removed the three dominant false-positive keywords in `src/lib/ky-topic-classifier.ts`: bare `emergency`/`safety` → Public Safety (matched "emergency clause" boilerplate; replaced with specific phrasing like `emergency management`/`first responder`/`public safety`), `commissioner` → Local Government (matched state offices like Commissioner of Agriculture / Insurance Commissioner; replaced with `magistrate`/`county judge-executive`), and `fiscal` → Taxation ("fiscal court/note/year" noise). Backfilled existing rows via `scripts/reclassify-bill-topics.ts` (22,547 scanned, 2,536 changed, ~2,548 bogus tags removed). Validated before/after on the LLM-flagged bills (SCR192/SB239/SB3/HB12). **Still open:** `HCR84` Energy vs. Natural Resources/Mining — no "Natural Resources" topic in the taxonomy; decide whether to add one. Remaining topic nuance is advisory (LLM) — see below.
- **LLM review pass (`llm-review.ts`) findings are advisory.** `ai_summary` faithfulness, `topics[]` plausibility, and glossary correctness must be verified against primary sources before edits. The model's own suggestions can be wrong (2026-06-03: it proposed incorrect veto/emergency-clause thresholds). Triage cadence for these flags is TBD.
