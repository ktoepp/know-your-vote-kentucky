...
- **Remove legacy event-database stack (2026-06-02)** — Finishes the deprecation started 2026-05-27 by deleting all orphan code that read the legacy `ky_meetings` schema or pointed back into it. **Net diff:** −5,000+ lines across 24 deleted files. Removed pages: `/events`, `/events/[id]`, `/activity`, `/explore`, `/link-dashboard`, `/live-content`, `/table`. Removed components: `BackNavigation`, `EventPreviewModal`, `SearchResultCard`, `TableView`, `LinkedEventsDisplay`, `EventDetailVerification`, `LinkedBillsDisplay`, `VideoGraphVisualization`, `StatusBar`, `PoliticalIntelligenceCard`, `BillListWithRefresh`. Removed libs: `useBillsData`, `useEventBillLinks`, `billStages`, `calendar-events`. All deleted files were orphaned (zero inbound references from current product surfaces — sitemap, SiteFooter, nav, /bills, /committees, /members, /meetings, /profile, /search, /feed). Lint warning count dropped from ~6 to 1 (sole remainder is `/find-content` — TASKS.md says "internal tool, left as-is"). Typecheck + build clean.
- **Remove legacy `/events` surface (2026-06-02)** — Deprecated 2026-05-27 (`noIndexMetadata`, dropped from sitemap, removed from in-app nav). Initial pass deleted `/events`, `/events/[id]`, `/events/layout.tsx`, and `BackNavigation` (only consumer was `/events/[id]`). The cascade follow-on (above) removed the rest of the legacy event-DB stack.
