...
(none — all 2026-06-02 PR train shipped; see Recently completed.)
...
- **LLM review pass (`llm-review.ts`) findings are advisory.** `ai_summary` faithfulness, `topics[]` plausibility, and glossary correctness must be verified against primary sources before edits. The model's own suggestions can be wrong (2026-06-03: it proposed incorrect veto/emergency-clause thresholds). Triage cadence for these flags is TBD. **Note:** the pass only LLM-audits `governmentTooltips`; the `topic_*` subject-topic + vote-count tooltips in the `voteCountTooltips` export are not reviewed, and `reviewSummaries` is a no-op while `ai_summary` is unpopulated.
...
- **Committee-membership deep reconciliation — not built.** Current legislator check compares contact fields only; full `committee_memberships[]` set reconciliation vs. Open States roles is deferred (normalization-heavy, false-positive-prone).
