...
- **`SCR192` mis-tagged "Public Safety".** The keyword classifier (`src/lib/ky-topic-classifier.ts`) maps the word "emergency" → Public Safety, so emergency-clause resolutions unrelated to public safety get the tag. Decide whether to scope the "emergency" keyword (e.g. require co-occurring public-safety terms) or move to LegiScan subjects / an LLM tagger. **Risk:** narrowing the keyword may drop genuine public-safety bills — needs a before/after diff on real data.
- **`HCR84` topic — Energy vs. Natural Resources/Mining.** No "Natural Resources" topic exists in the taxonomy; classifier forces a near-miss. Decide whether to add a topic or accept the current mapping.
