...
- **Remove legacy `/events` surface (2026-06-02)** — Deprecated 2026-05-27 (`noIndexMetadata`, dropped from sitemap, removed from in-app nav). This pass finishes the job: deleted `src/app/events/page.tsx`, `src/app/events/[id]/page.tsx`, `src/app/events/layout.tsx`, and `src/app/components/BackNavigation.tsx` (only consumer was `/events/[id]`). Build + typecheck + lint clean. **Cascade left for a separate cleanup:** `/table`, `/live-content`, `/link-dashboard`, `EventPreviewModal`, `SearchResultCard`, `TableView`, `LinkedEventsDisplay`, `EventDetailVerification`, `useEventBillLinks`, `calendar-events.ts` still exist and reference `/events/${id}` — they're all unlinked from the public surface (not in sitemap / footer / nav) so dead links to the now-deleted `/events/${id}` only matter to internal users browsing those orphans. A follow-up PR can remove the full legacy event-database stack.
