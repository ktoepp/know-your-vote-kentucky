...
- **Status mapper doesn't preserve furthest progress (real, low-frequency).** Surfaced by the bills checker on HB21: stored `Engrossed`, but the latest LegiScan action is `To: Interim Joint Committee on Health and Welfare`, which `mapLegiScanBillStatus` maps back to `In Committee`. A bill that passed a chamber and was then referred to an interim committee gets "downgraded" because the mapper keys off the latest action text, not the highest milestone reached. Decide whether `ky_bills.status` should reflect *furthest progress* (so a fresh re-sync wouldn't regress `Engrossed` → `In Committee`). Affects both the sync and the audit. ~1/25 sampled bills.
