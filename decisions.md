...
## 2026-06-12 — Committee record merge (seed-code vs LRC full-label duplicates)

**Fixes the top finding from the same-day design+a11y audit.** `ky_committees` upserts on `(lrc_rsn, committee_type)`, and the LRC changed the `CommitteeType` URL param on the legislative calendar from short codes (`IJ`, `S` — the values migration 027's seed also used) to full labels (`Interim Joint Committee`, `Statutory Committee`). Every interim/statutory committee then got a second row on the next sync: **14 confirmed pairs** (not the 2 suspected during the audit), each sharing an `lrc_rsn` with data split across the rows — old row held the PDF-backfilled future meetings and any follows; new row received current calendar scrapes and materials copies.

- **Survivor rule: the row current LRC syncs target.** Verified live: the calendar today emits full-label `CommitteeType` values, so the full-label row survives every merge. Merging the other direction would let the next sync recreate the duplicate. The materials sync (keyed on `lrc_rsn` over *all* rows) had written identical material sets to both rows of each pair — loser copies were conflict-deleted, not moved.
- **Meeting collision matching is date-only, deliberately.** The loser's meetings carry terse PDF-backfill time strings ("11:00 ET"); the survivor's calendar-scraped rows carry the full form ("11:00 am ET / 10:00 am CT, Annex Room 154") — same real-world meeting, so `(date, time_and_location)` equality would have produced double-listed meetings and later false `meeting_cancelled` digest events (one had already fired: the loser's 2026-06-02 education meeting was stale-flipped to cancelled when the survivor took over the sync). On collision the survivor's row wins; the loser meeting's agenda items are dropped when the survivor already has agenda content, and its materials/events re-point with dedupe-conflict handling.
- **Aliases over hard 404s (migration 030).** `ky_committees.aliases TEXT[]` + GIN; merge appends the loser slug to the survivor; `/committees/[slug]` falls back to an alias lookup and issues a `permanentRedirect` (308). Old bookmarked URLs (e.g. `/committees/admin-regs-review`) keep working.
- **Idempotent + auditable.** `merge:duplicate-committees` is dry-run by default (`--live` to write, explicit `--pair=loser:survivor` overrides, auto mode only merges clean same-rsn short/full-label splits and skips anything ambiguous); live runs write a JSON change report under `reports/`. Re-run after apply: "No mergeable pairs found."
- **Applied to primary 2026-06-12:** 14 pairs, 244 actions; `ky_committees` 69 → 55 rows; the one real follower's 3 follows moved with per-user dedupe; `committee_memberships` arrays had no loser-slug references (interim committees aren't in the LRC-roster backfill set). Verified locally: canonical admin-regs page shows 8 members *and* the Jul 8 upcoming meeting; alias slug 308s to it.
- **Regression guard:** `audit:accuracy` checkCommittees now warns when two rows share an `lrc_rsn` or a normalized (depluralized) name, before the calendar fetch so it fires even when LRC is down. The 3 remaining "suspect" diagnostic pairs (House standing budget-review subs vs interim joint ones, different RSNs) are genuinely different committees and stay unflagged.

**Revisit if:** LRC changes the `CommitteeType` param again (the weekly warn will catch it — run the merge after confirming direction); a committee genuinely meets twice on one date during a merge window (date-only collision matching would fold them — acceptable for interim bodies, recheck before merging session-period standing committees); fresh-DB installs re-seed 027's short codes and immediately duplicate (the merge script handles it, but consider updating the 027 seed values to full labels for new environments).
