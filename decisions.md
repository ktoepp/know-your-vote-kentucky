...
## 2026-07-20 — legislator link verify: treat legislature.ky.gov 503 / status-0 as transient, not a failure

The weekly `legislator-links-weekly` verify failed with **79/421 "failures," every one an `lrc_profile_url` on `legislature.ky.gov` returning HTTP 503 or status 0** — no 404s. Run history shows the same identical `…/Legislator-Profile.aspx?DistrictNumber=N` URLs pass most weeks and fail ~1 in 4–5 (07-20 ❌, 07-18 ✅, 07-13 ✅, 07-06 ✅, 06-29 ❌, 06-01 ❌). That signature — intermittent, host-wide, 503/0 never 404 — is the IIS host **throttling the probe's concurrent burst**, not broken links. The verifier was paging `#errors` on a transient blip.

- **Root cause in code.** `verify-legislator-external-links.ts` `probeUrl` only fell back to GET on `405/501`/network-throw — a `503` was recorded as a hard fail with **no retry, no backoff, no jitter** at `CONCURRENCY = 6`, which also *provoked* the throttle. Any non-2xx/3xx outside the three existing exemptions (LegiScan-403, social bot-block, quota-hold) counted toward `failed` → exit 1 → Slack `#errors`.
- **Fix = the principle the sibling probe already codified.** `classifyLinkStatus` (`ky-committee-material-link-probe.ts`, § 2026-06-13) records **404 → dead, 2xx/3xx → ok, everything else (5xx/timeout/status-0) → inconclusive** so "a transient blip or bot-block never flips a good link to dead." The legislator verifier predated that lesson; it now matches it: (1) **retry** transient results (`isTransientStatus` = status 0 or 5xx) up to `MAX_PROBE_ATTEMPTS = 3` with exponential backoff (`RETRY_BASE_MS = 800` ×2^n) + jitter, plus a small pre-probe jitter to spread the burst; (2) any transient result **surviving** retries is recorded as a new **`skip` (transient/server-unavailable)** category — `exemptTransient` — counted and surfaced in the summary/JSON (`skippedTransient`, `verifierNote: transient_server_unavailable_exempt`) and Slack stats, but **not** counted as `failed` and **not** paged. Only a definitive **404** (dead link) still fails the job.
- **Trade-off / revisit.** A genuine host-wide outage now turns the job **green with a large `skippedTransient` count** rather than red — the count is in the Slack summary so it stays visible, matching the committee-probe's stance. If we later want a real outage to page, gate on a threshold (e.g. `skippedTransient` > X% of probes) rather than reverting to per-probe failure. Verified end-to-end against a local mock (503→503→200 recovers on attempt 3; always-503 gives up after exactly 3 tries and classifies as skip; 200/404 return on the first request with no retry; 404 still classifies as a real failure).

---

## 2026-07-22 — Adopted simple resolutions coded "Introduced" read as "Adjourned Sine Die" (SR252)

The progress meter + status chip at the top of a bill page/card showed **"Adjourned Sine Die"** for Senate resolutions whose detailed timeline / voting record clearly showed them **adopted** (accuracy finding: SR252 2026 RS — meter "Adjourned", history "adopted 38-0", a `ky_votes` roll call 38-0 `passed=true`).

- **Root cause: a LegiScan data gap, not a rendering bug.** KY disposes of a simple resolution with a single adoption vote. LegiScan tags that adoption action with **`importance: 0`** and — for **roll-call** adoptions ("adopted 38-0") — leaves the bill coded as **status 1 (Introduced)**, whereas **voice-vote** adoptions of the same resolution are stored as **"Passed"**. So `bill.status` understated the roll-call form. Once the session ended, the § 2026-06-10 sine-die override (`sessionHasEnded && isActivePendingBillStatus`) then flipped the stale "Introduced" to "Adjourned Sine Die". The detailed **HistoryTimeline** reads the raw history/votes, so it showed the adoption — hence the top-vs-detail disagreement. Scope: **922** simple-resolution rows had a pending status with an adoption-shaped last action.
- **Pure display-layer fix — no DB sync, durable across resyncs.** New `effectiveKyBillDisplayStatus(bill)` in `bill-display.ts` reports **"Passed"** (matching the voice-vote form already stored) when a **simple resolution** (SR/HR) has a last action that records adoption **and** its stored status still understates it (blank or pending). Adoption is detected by `actionIndicatesResolutionAdopted` — deliberately narrow: it requires an explicit vote outcome (a `<yea>-<nay>` tally or "by voice vote") and rejects procedural text ("committee substitute adopted", "floor amendment adopted", "passed over…"), which **preserves the SR231 regression guard** (a resolution truly pending at sine die must not read as adopted). Every other bill returns its stored status unchanged.
- **Applied at the two derivation points** so the meter, the chip, cards, the detail page, and the digest all agree: `getBillProgress` (`ky-bill-progress.ts`) derives its bucket/adjourned/label/adopted checks from the corrected status; `BillStatusMetaChip` corrects `rawStatus` the same way (it also composes with the detail page's history-derived `statusOverride`). Joint/concurrent resolutions and regular bills are **untouched** — a one-chamber adoption is not final for a two-chamber measure, and regular bills already get correct passage status from LegiScan.
- **Verified** against representative cases: SR252 roll-call → "Passed" / fully adopted; voice-vote SR → "Passed" (unchanged); "committee substitute adopted" & "passed over" & truly-in-committee SRs → still "Adjourned Sine Die"; regular introduced bill at sine die → still "Adjourned Sine Die"; adopted SR mid-live-session → "Passed".

---

## 2026-07-22 — Accuracy audit: transient upstream outage is a skip, not an operational error; last_action false-flag fixed

The weekly audit run **2026-07-19 went red on `#errors`** for a non-event: the `legislators` checker threw `OpenStates API 504: … Gateway Time-out …` after the client's own 3 retries, and a checker that throws sets `hasOperationalError` → exit 1 → page. Open States 504s are a known intermittent condition on the cold, heavy `/people` endpoint (the client comment already says so, `timeout: 90_000`) — an upstream we can't control, not our bug. This is the same lesson as the LegiScan quota stop (§ 2026-06-28) and the legislator-link verifier's 503/status-0 handling (§ 2026-07-20): **expected/transient upstream conditions skip quietly; only a genuine bug pages.**

- **`isTransientUpstreamError(e)`** (`accuracy-audit/types.ts`) is the shared predicate: HTTP **5xx / 429**, a request **timeout**, or a low-level **network** code (`ECONNRESET/ETIMEDOUT/ENOTFOUND/…`) → transient. It reads the structured axios shape (`response.status`, `code`) first, then falls back to the message text for errors that wrap the status as a string (the Open States client throws `Error("OpenStates API 504: …")`). Auth **4xx** (401/403/422), a **404** (URL moved), and ordinary logic/schema errors are **not** transient and still page. Unit-checked against the real 07-19 strings plus genuine-error counterexamples.
- **Two whole-run-on-one-fetch checkers now skip instead of erroring** when that single upstream fetch fails transiently: `legislators` (Open States roster) and `committees` (LRC live calendar). Both return `{ skipped: true, skipReason }` — a quiet `skipped` domain line in the digest, no `#errors`, exit 0. The `committees` near-duplicate-row findings gathered *before* the calendar fetch still report. `bills`/`votes` (per-item LegiScan failures → `warn`) and `materials` (per-committee LRC failures → `warn`, outer `try/catch`) were already resilient — unchanged.
- **`bills` last_action false-flag removed.** The checker reconstructs last_action from `getBill`'s `history[]`, but the sync stores it from `getMasterList`/`getSearch`, which phrases the **same** action differently ("To: Interim Joint Committee on Appropriations and Revenue" vs "to Appropriations & Revenue (H)" — HB48, 07-19 run). A raw string diff false-flags on phrasing alone. `status` (a `fail`, via the tolerant status mapper) is the reliable staleness signal; the last_action `warn` now fires **only when status also diverged**, as supporting context. tsc + eslint clean.
- **Not touched here (genuine findings, need data ops — carried as follow-ups):** the 07-19 warns for near-duplicate committee rows (`lrc_rsn=26`, `lrc_rsn=9` → `npm run merge:duplicate-committees`), topic false-positives (HB84 guardianship→Healthcare, HB142 tax→Agriculture/Education/Environment, SR35 honorary→Voting Rights → classifier tuning + `topics:reclassify`), and glossary copy ("Kentucky General Assembly" annual-session wording, "Governor's Action" title/status mismatch). These are correct catches, not audit noise.

**Revisit if:** a genuine sustained upstream outage should page — gate on a threshold (e.g. N consecutive weeks skipped) rather than reverting to per-fetch error, matching the § 2026-07-20 stance.

---

## 2026-07-22 — New "Honors & Memorials" topic for ceremonial resolutions (accuracy-audit follow-up)

The weekly LLM topic review flagged **SR35 2023** ("A RESOLUTION honoring Congressman Morgan McGarvey upon his election to the United States House of Representatives") as **Voting Rights** — the keyword classifier hit `election` in a congratulatory phrase. Rather than suppress the tag, we route ceremonial resolutions to a dedicated, browsable category so honorariums and in-memoriams are searchable as a group.

- **New tag `Honors & Memorials`** (23rd entry in `KY_TOPICS`). Assigned **structurally from the title**, never from body keywords — `classifyTopics` short-circuits to `['Honors & Memorials']` when `isCeremonialResolution(title)` matches, skipping keyword matching entirely (the honoree's district / profession / "election" describe the person, not a policy area). `TOPIC_KEYWORDS['Honors & Memorials']` is intentionally empty for the same reason.
- **Detection** (`isCeremonialResolution`, `ky-topic-classifier.ts`): a lead-verb pattern (`honoring/recognizing/congratulating/commemorating/celebrating/mourning/memorializing`, with or without the `A RESOLUTION` prefix) **or** an `in (loving) honor|memory` phrase. This mirrors the ceremonial half of the search demotion (`SEARCH_CEREMONIAL_RESOLUTION_RE`, `ky-search-bills.ts`) but is widened to the short LRC title forms the classifier also sees ("Adjourn in honor and loving memory of …", "Honor …", "Memorialize …"). `designating`/appointment resolutions are **excluded** — demoted in search but not honors/memorials, so they fall through to normal classification. Substantive resolutions ("urging Congress …", "directing the LRC to study …") do not match and keep their policy topics.
- **Fully wired as a category:** the tag flows into the topic index / landing pages (`KY_TOPIC_INTROS`, `kyTopicSlug` → `/bills/topics/honors-memorials`), notification follows (`ky-notification-preferences` validates against `KY_TOPICS`), and the sitemap — all enumerate `KY_TOPICS`, so no per-surface wiring was needed beyond the three exhaustive `Record<KYTopicTag, …>` maps (keywords, intros, LegiScan-subject bridge — the last intentionally empty, as LegiScan does not subject-code ceremonial resolutions).
- **Backfill required** (like every prior classifier change): the sync only reclassifies new/changed bills, so existing ceremonial rows keep their old tags until `npm run topics:reclassify` runs (dry-run first). This both drops the stale mislabels (SR35 → Voting Rights) and populates the new category across historical resolutions.
- **Audit traceability fix (same branch):** the LLM summary/topics passes now key findings by `<bill_number> · <session>` instead of bare `bill_number` — 17 different "HB142" rows span 2010-2026, so the old key was neither actionable nor collision-free within a sample.

**Revisit if:** we want ceremonial resolutions to also retain a genuine secondary policy topic (currently they get only the structural tag), or want `designating`/awareness-day resolutions folded in.

## 2026-07-24 — Bill popular / colloquial names (PR #200)

Users recognize bills by their common names ("Safer Kentucky Act," "Marsy's Law," "the bathroom bill") far more than by `HB 5`, but `ky_bills` only stored the formal `title`, so search, digests, and bill pages couldn't match or show the names people actually use. This feature captures popular names and threads them through sync → search → display. Full spec + built steps: [docs/specs/bill-popular-names.md](./docs/specs/bill-popular-names.md).

- **Governing constraint is voice-and-tone (non-partisan, honest sourcing): attribute, never adopt.** We show a name and, for official ones, where it comes from — we never let a loaded media nickname read as KYVKY's own characterization of a bill.
- **Two classes, stored and displayed separately by provenance / neutrality risk.** `official_short_titles TEXT[]` from the Kentucky LRC "Short Titles and Popular Names" list (neutral, official, sourceable); `editorial_popular_names TEXT[]` — media/advocacy names in no official list, **human-curated by Katie** (the editorial gate *is* the neutrality control; no automated harvesting). Bill page labels media names **"Also called"** with no disclaimer text.
- **Zero LegiScan quota.** LegiScan has no popular-name field (verified), so official names come from an LRC scrape (`src/lib/lrc-popular-names-parser.ts` + `npm run sync:ky:lrc-popular-names`), reusing the enrollment-actions scraper pattern.
- **Search matches both classes, normalization-tolerant** (punctuation, apostrophes, casing, spacing, leading "the") with light spelling tolerance; both arrays join the `search_vector` generated column at weight B (same tier as `topics` / `ai_summary`). Migrations **043/044** applied + verified on primary; 2025 RS bootstrapped from the fixture (a live LRC sync run is egress-blocked from the build env — Katie to run it).
- **Digest: media names now included, attributed.** Initially deferred to official-only; **resolved (Katie, 2026-07-24) to include them** — the digest shows an attributed "Also called:" line from `editorial_popular_names` alongside the official short title.

**Revisit if:** editorial curation volume outgrows manual entry (would need a review workflow, not automated harvest), or the LRC changes its short-titles page structure.

## 2026-07-24 — Dictated UI-fixes batch: favicon circle, /districts tile grid, three-column footer (PR #201)

A dictated round of UI refinements shipped alongside the design-system v1.1 work. Record kept in TASKS.md → Recently completed; the notable "why" calls:

- **Favicon refined from a rounded square to a circle, in a darker blue `#184D8F` — reverses the v1.1 white-on-blue rounded-square favicon (§ design-system v1.1).** The circle reads more distinctly at tab scale, and `#184D8F` is the logo blue `#1C5BAA` a few points darker in lightness so white-on-fill clears ≈8.9:1. Generated reproducibly by the committed, idempotent `scripts/generate-favicons.py` from `public/branding/mark-color-512.png` (single input-of-record) → `public/favicon.ico` + the `public/branding/*` set + the `src/app/{icon,apple-icon}.png` file-convention fallbacks. `apple-touch-icon`/`apple-icon` stay **full-bleed squares** (iOS applies its own squircle mask). **Fixed in passing:** removed the stock `src/app/favicon.ico` (present since the first commit) that collided with the branded `public/favicon.ico` — the dev server 500s on `/favicon.ico` and the stale default won in prod.
- **`/districts` index → responsive tile grid, every tile deep-links to the map.** The two-column text list became a House/Senate tile grid; all 138 tiles link to `/members/map?chamber=…&district=…` via a new `kyDistrictMapPath()` helper so a click lands on the interactive map with that district preselected. `/districts/[slug]` detail pages are **not** orphaned — member profiles still link to them and they stay in the sitemap.
- **Footer → three labeled `nav` columns** (Explore / Account / Learn more), each an `aria-label`led landmark with an overline heading (two-up on xs, three-up from sm).
- **Masthead + footer wordmark made non-selectable / non-draggable** (`user-select:none` + `draggable={false}`) so drag-select across the header no longer highlights it and the mark can't be dragged out as a file.

## 2026-07-24 — Design system v1.1 code migration complete (PR #202)

Milestone marker for the append-only timeline; the authoritative record is [design-system/audit.md](./design-system/audit.md) + [design-system/cleanup.md](./design-system/cleanup.md). The v1.1 code migration is fully applied: AA contrast promotions (`--success #15803D` / `--warning #B45309`), the `text.tertiary` metadata role + read-metadata migration, distinct party colors (indigo/rose/slate, unified with `bill-display.ts`), back-compat CSS alias retirement (Phase 4), and the radius reconciliation (Phase 3 — kept the 24px CivicCard look, named it `--radius-lg` / `rounded-card`, zero visual change). `tsc` + `lint` clean. **Only remaining open items:** font self-hosting (Aesthet Nova licensing — audit open decision #1, deferred) and the v1.2 dense-table archetype (open decision #4, deferred to v1.2). The white-knockout logo + favicon (open decision #3) is delivered and wired.

## 2026-07-27 — Cleared the residual dev/tooling `npm audit` advisories (9 → 0)

Same-day follow-on to the prod-bundle security pass (PR #206, which took audit 18 → 9 and deferred the rest). The residual 9 high advisories were all dev/tooling-only and each carried a real reason for deferral; both are now resolved with surgical, reversible changes. `tsc --noEmit` / `next lint` / `next build` clean throughout; `npm audit` ends at **0**.

- **`adm-zip ^0.5.17 → ^0.6.0`** (GHSA-xcpc-8h2w-3j85, crafted-ZIP 4 GB-allocation DoS). **Deferral note corrected:** adm-zip is *not* confined to gitignored scripts — it's imported by the tracked `src/lib/ky-legiscan-dataset-import.ts`, reached by `sync:ky:dataset` (weekly `legiscan-dataset-weekly.yml` Action) and `bulk-seed:ky`. But it is only the **fallback** ZIP parser (the primary path is the native-zlib `iterateZipEntries`, which handles LegiScan's store/deflate datasets), and it never enters the Next.js prod bundle — no `app/` or API route imports that module. The 0.5→0.6 breaking changes are limited to `extractEntryTo()` sub-directory preservation and best-effort utimes; the surface this repo uses (`new AdmZip(buf)` / `.getEntries()` / `.entryName` / `.isDirectory` / `.getData()`) is unchanged, verified with an isolated round-trip smoke test. So the fix is a clean bump, not a code-migration.
- **`overrides` brace-expansion pin `1.1.13` → `5.0.8` (blanket), replacing the scoped `minimatch@3.1.5 → brace-expansion` form.** Root cause of the whole eslint-devtree cluster (8 of the 9 advisories chained through `minimatch`): a newly-disclosed advisory **GHSA-mh99-v99m-4gvg / CVE-2026-14257** (unbounded-expansion OOM) publishes its affected range as a **flat `<= 5.0.7`**, so npm audit flags *every* brace-expansion below 5.0.8 regardless of major line — the prior scoped `1.1.13` pin, and even the `1.1.16`/`2.1.2` maintenance backports, can no longer satisfy it. **Why a blanket pin is safe here where it normally wouldn't be:** brace-expansion's single-function API is stable across majors, 5.0.8 ships a CJS `require` export, and the only consumers are `minimatch` 3.x/9.x/10.x used by dev-only lint tooling — `next lint` passing is the runtime proof. A single `"brace-expansion": "5.0.8"` dedupes the three tree copies (1.1.13 via `@eslint/eslintrc`, 2.1.0 via `@typescript-eslint`, 5.0.7 via `react-email`→`glob`) into one node. This **deliberately avoids** the alternative — letting `npm audit fix` drag the whole `eslint`/`@typescript-eslint` devtree, which historically **nets +1** advisory. Net lockfile effect is a **reduction** (−35 lines: the orphaned `balanced-match@1.0.2` / `concat-map` copies drop out); no `eslint`/`typescript-eslint`/`react-email` version drift.
...
## 2026-07-31 — Health check: Vercel MCP gap recurs; bills-browse timeout hypothesis narrowed to a candidate index

Scheduled health check across connected monitoring services (GitHub Actions + Vercel MCP, per the daily "System health check" Routine).

- **Vercel MCP tools didn't load this session** despite `ListConnectors` showing the connector installed and `enabledInChat: true` — no `mcp__Vercel__*` tools were reachable via targeted `ToolSearch` queries (deployments, projects, logs, observability). Same class of gap as § 2026-06-09, § 2026-06-23, and the self-healed 2026-07-20 occurrence; § 2026-07-06 traced one prior instance to a missing team grant on the connector. Net effect: no Vercel deployment status or Sentry-surfaced runtime-error counts for the `ky_bills browse query failed` watch item could be pulled this check. GitHub Actions access was unaffected — all 6 workflows confirmed green on their latest runs, 0 open issues, 1 open PR (the rolling health-check PR).
- **`ky_bills browse query failed [count]` watch (open since 07-28, escalated 07-30)** — with no fresh telemetry available, used the gap to read the actual query instead. `fetchKyBillsBrowsePageUncached` (`src/lib/ky-bills-browse-server.ts`) — on the default no-filter path that `/bills`, `/members/[slug]`, and `/meetings` all hit — runs `ORDER BY session DESC, last_action_date DESC NULLS LAST` plus an exact `count()` over the full `ky_bills` table. `supabase/migrations/002_indexes_and_rls.sql` only indexes `session` and `last_action_date` separately (`idx_bills_session`, `idx_bills_last_action_date`); there is no composite index covering both sort keys together, so Postgres can't satisfy the two-column ORDER BY from an index scan and likely falls back to sorting the matching rows directly — a cost that grows every sync cycle as the current session's row count grows, structurally identical to the `ky_votes.roll_call` JSONB-scan bug PR #212 fixed via `045_get_votes_for_legislator_perf.sql`.
- **Not shipping a migration on this alone.** No `EXPLAIN ANALYZE` access this session (no Supabase/DB connector available) to confirm the planner is actually doing a full sort rather than something else (lock contention, pool exhaustion, etc.). Recorded as a concrete candidate rather than another "re-check tomorrow": if profiling confirms it, add `CREATE INDEX ky_bills_session_last_action_idx ON ky_bills (session DESC, last_action_date DESC NULLS LAST);`.

**Revisit if:** the Vercel MCP gap recurs again on the next scheduled check — three prior self-healed occurrences plus this one is enough to stop assuming it'll fix itself; worth checking whether the daily Routine's env (`env_011CUP9uJd4u7xGxxR4Sw2EE`) has a stabler grant than ad hoc sessions, or moving Vercel/Sentry checks to a manual dashboard cadence as floated 2026-06-23. Also revisit once the bills-browse query has actually been profiled — either confirming the composite-index fix or ruling it out.
...
## 2026-08-11 — Daily health check: the 2026-08-10 "blank-agenda backlog cleared on its own" retraction

**What the 08-10 entry claimed:** a re-verification query found 0 meetings in the (non-empty `agenda_content_hash` AND zero `ky_committee_agenda_items` rows) state, and attributed the historical 84-meeting backlog to the weekly LRC sync re-deriving items over time as hashes changed — no code fix, no repair script.

**What actually happened, same evening.** ktoepp manually triggered `accuracy-audit.yml` (run `31427099436`, `seed=1072134505`) at 20:02 UTC, a few hours after that entry was written. It found the identical defect signature — `agenda_content_hash records agenda text but no ky_committee_agenda_items rows are stored (agenda lost on write)` — on **81 of 408** committees checked (plus 9 `sort_order`-gap warns, the same underlying symptom). The dates span 2024-12 through 2026-08-06, i.e. both the "already-resolved" historical backlog and current-week meetings. The 08-10 verification query was real but scoped wrong, or ran against a state that had already regressed by evening — either way, "cleared on its own" was false, and the backlog was never self-healing.

**Root cause, found and fixed same day (PR #238):** `ky-lrc-calendar-sync.ts` replaced a meeting's agenda by issuing a `DELETE` and then an `INSERT` as two independent PostgREST calls. Any insert failure — a network blip, a bad row — left the delete committed and the meeting silently empty, while `agenda_content_hash` (written separately, unconditionally) kept recording the correct source text. This is exactly the shape of bug the "cleared on its own" theory couldn't distinguish from a genuinely-fixed backlog: a query for "current state" can't tell a permanently-resolved row from one that will drop its agenda again on the next sync that happens to hit a transient insert failure.

**Fix:** migration **050** (`supabase/migrations/050_ky_replace_agenda_items_rpc.sql`) adds `ky_replace_committee_agenda_items(p_meeting_id uuid, p_rows jsonb)`, a `SECURITY DEFINER` plpgsql function that performs the delete and insert inside one Postgres transaction — a failed insert now rolls the delete back, so the previous agenda survives instead of being silently emptied. The sync (`src/lib/ky-lrc-calendar-sync.ts:421`) now calls this RPC instead of two raw calls. Same-day PR #239 separately cleared the run's 8 remaining LLM-review warns (audience-hedge overreach in three `ai_summary`s, four bill `topics` misclassifications, and two glossary citation/precision fixes). Both merged; production (`dpl_HRo62vVMEjy2kUoFXmWqvhF9s5ua`, 2026-08-10 23:41 UTC) has the fix.

**Open verification gap, carried to the next check.** Migration 050 is a schema change and this repo's migrations are applied manually (`npm run db:apply-sql` / SQL editor) — a Vercel code deploy does not apply it. As of this check (2026-08-11 ~12:20 UTC) no `sync-lrc-calendar` run has executed since the fix deployed (last run 08-10 18:57 UTC predates the 20:55 UTC deploy; the 08-11 12:00 UTC scheduled run was ~20 min late, not yet alarming). **If migration 050 was not applied, every sync will now hard-error** on `db.rpc('ky_replace_committee_agenda_items', …)` with a "could not find function" PostgREST error — worse than the old silent-partial-failure behavior, since it fails every meeting rather than occasionally one. Next check: read the next `sync-lrc-calendar` run's logs for that specific error, and/or re-run `accuracy-audit.yml`'s committees checker to confirm the fail count actually dropped from 81.

**Method note for this routine, going forward:** a "0 rows in this bad state" query is not sufficient evidence that a write-path bug is fixed — it only proves the *current* state is clean, which a still-broken write path will produce by coincidence whenever nothing has failed *yet*. Closing a defect like this needs either (a) a code-level fix to the write path itself, or (b) if genuinely believed self-healing, a repro across multiple sync cycles under conditions that previously triggered it (e.g. injected insert failures) — not a single point-in-time count.

### Also this check (not code issues, no action)
...
## 2026-08-22 — First-person copy on `/about` and in the welcome email

Katie asked to "open up a bit" about the project — how it was made and who made it — on the two surfaces where a newcomer first meets it. Source material came from the Notion strategic hub (Master Narrative, Messaging & Descriptions, Partnership & Distribution Targets, the LinkedIn positioning doc); every claim was re-verified against the repo before it shipped.

**Two surfaces, deliberately different lengths.** `/about` carries the full account in three new sections — **Why this exists**, **Who builds it**, **How bill summaries are written** — placed above the existing Data sources card so the human context arrives before the plumbing. The welcome email gets only a short signed note between the last card and the footer, with **More about the project →** carrying readers to `/about`. Rationale: the email is transactional and its job is orientation; a founder essay in a setup email is the kind of thing that trains people to skim the next one. Short there, complete on the site.

**Why first person at all.** The voice guide's **Honest sourcing** principle already says our willingness to state limitations is a trust asset. "A solo, unfunded person built this and here is what it costs" is the same move applied to provenance rather than data. A stranger evaluating a non-partisan civic tool asks *who is behind this and what do they want* before they trust a bill summary; answering plainly is cheaper than being asked. The personal register is now documented in `docs/voice-and-tone.md` under **First person: when the project speaks as a person**, scoped to exactly these two blocks — everywhere else the project still says "we" or nothing at all.

**Claims verified in the repo, not copied from Notion.**
- *"It never characterizes a legislator"* — checked against `SYSTEM_PROMPT` in `src/lib/ky-content-generation.ts` and the generator's export surface: every target is a bill, ordinance, executive order, or school-board action. No member-summary path exists. This is the one accuracy claim that holds today because it is structural rather than promised, so it is stated flatly with no hedge.
- *"Replies to this email reach me"* — `replyTo: 'katie@kyvky.com'` is set on all three send paths (`src/app/api/me/welcome-email/route.tsx`, `src/lib/digest/run-bill-digest-cron.tsx`, `scripts/preview-welcome-email.tsx`). If a future send ever drops reply-to, this line has to come out with it.
- *"the code is open source"* — `ktoepp/know-your-vote-kentucky` is public, MIT, and already surfaced on `/licenses`.
- *"Summaries cover every bill from the 2024 session forward"* — the sanctioned phrasing from the Notion wording rules; 2024/2025/2026 are each at 100%, 2023 and earlier are indexed without summaries. Never state the raw all-sessions percentage without the session split.

**Wording rules carried over from the funder-facing docs**, because they are the same claims and must not diverge between a grant application and the public site: infrastructure cost is stated as *"about $1,000 a year to run; the work behind it has been contributed rather than paid"* (never "the project runs on $1,000 a year," which prices the labor at zero); coverage is stated as an *aim* ("a whole session rather than a curated selection"), never as achieved; "live since February 2026," never "a full session."

**The AI-build disclosure is soft on purpose.** "Directing AI tools along the way" — a subordinate clause inside a sentence about building the thing, not a headline. Notion's tone note keeps "vibe-coding" for journalists, where it reads as authentic, and softens it for advocacy and public audiences. Disclosing it at all is consistent with the page already disclosing that summaries are model-written; hiding it while disclosing the other would be the odd choice.

**Deliberately excluded.** No funding status, grant ask, or fiscal-sponsorship language — that belongs in outreach, never in product copy. No named former employers: the Notion material names specific institutions, but those are funder-facing credentials and the exact names could not be verified from the repo, so the background is described by kind ("museum interpretation first, then brand and product work for organizations in democracy and media literacy") and left for Katie to make specific if she wants. No claim about effect on turnout or participation.

**Voice tension accepted.** The page mixes institutional "we" (intro, Data sources, Email alerts) with "I" inside **Who builds it**. The section opens by naming the project as one person's, which makes the switch self-explaining. Converting the whole page to first person is the alternative if the mix ever reads as sloppy rather than as an aside.

`docs/voice-and-tone.md` gains the welcome-email note spec and the personal-register section; `src/app/design-system/page.tsx` gains the note in the welcome-email specimen so the pattern gallery does not drift from the shipped email.

**Punctuation revision, same day.** Katie asked for no em dashes. Applied to `/about` and the welcome email in full, including the eight instances that predated this change, since leaving them would have made the new sections visibly inconsistent with the stated preference. Replacements are a colon where the second half explains the first, a comma for a light aside, and a full stop where the clause stands alone; no meaning changed. The guide's quoted welcome-email card specs and the design-system specimen were updated in the same pass so canon does not drift from what ships. **Not swept sitewide** — the digest email, glossary, session banners, and browse surfaces still use em dashes, as do the digest specs in `docs/voice-and-tone.md`. Recorded as a convention there with that gap named explicitly.

**Founder note replaced with Katie's own copy, same day.** She rewrote the welcome-email note herself and it ships close to verbatim. Only typo-level fixes were applied: `undersatnding` → `understanding`; `wall-` → `wall:` (the hyphen was standing in for a dash, which the new punctuation rule bars); `confusing/outdated ux` → `confusing and outdated interface` (the guide bars jargon in user-facing copy, and the slash reads as a note-to-self); `as easy as it was to follow` → `as easily as`; `Currently seeking` → `I am currently seeking` (sentence fragment); terminal period added. Sentence order, word choice, and rhythm are hers and were left alone. The guide now marks this block as **authored text** so a later copy pass does not quietly normalize it toward the house register.

**Reversal: funding language is allowed in product copy.** The entry above recorded "no fundraising in product copy" as a rule; Katie overruled it the same day and the guide has been corrected rather than left contradicting what ships. Her reasoning stands on the existing Honest-sourcing principle: a solo unfunded project saying so plainly is the same move as admitting the topic tags mislabel bills. The constraint that survives is tense, not topic. "I am currently seeking sponsorship for grant funding" is checkable today; naming a sponsor or implying one is secured stays barred by the Notion wording rules until an agreement is signed, at which point the line changes in three places together (email, `/about`, design-system specimen).

**`/about` realigned to the same story.** Her note supplied the real origin (knowledge is power; the paywall-or-bad-interface wall; wanting legislation to be as followable as a feed), which replaced the placeholder background sentence the earlier pass had invented from the Notion positioning doc and flagged as unverified. "Designer and developer" became "designer and self-taught developer" per her own description, and the funding sentence was added so the two surfaces do not tell different stories. The unverified employer history is now gone entirely rather than sitting on the page awaiting confirmation.

**Brand stylization corrected.** `src/lib/structured-data.ts` (`alternateName: 'KYvKY'`) and `llms.txt` already declared the lowercase-v form, but four user-facing strings still rendered all-caps "KYVKY" — two on `/about`, two on `/legislature/resources`. Katie's own copy uses KYvKY, so the public site was rendering the brand two ways. Fixed in all four and recorded as a convention. Remaining all-caps instances are code comments, internal audit output, and the design-system reference page, none of which a visitor sees.

**Third pass, same day: greeting, funding tense, brand sweep.**

*Greeting.* "Thank you for using KYvKY" was inaccurate in an email that fires at verification, before the reader has used anything. Now "Thank you for signing up!" on its own line above the body, with the exclamation point Katie asked for. This is the one sanctioned exception to **Warmth through anticipation, not enthusiasm** and the guide says so explicitly, scoped to this greeting alone. The sign-off still reads "Thanks again for using KYvKY," which is correct in that position: it is a send-off, not a claim about what the reader has already done.

*Funding narrowed from ask to note.* The previous pass allowed funding language in the "seeking" tense. Katie narrowed it further: note the situation, do not solicit. "I am currently seeking sponsorship for grant funding that will add expertise and make the project scalable and sustainable" was true but reads as a pitch, so it is out. In its place, "Right now it is a passion project, and I am working to fund and grow it" — her own phrasing, lifted from the outreach drafts in Notion. The operative test recorded in the guide is whether the sentence asks the reader for something. Same change on `/about` so the surfaces match.

*Rhythm.* One structural edit beyond the greeting: the long first sentence of the wall paragraph is split at "But I kept hitting a wall," which lets the paywall/interface pair land on its own. Word choice was left alone by request; suggestions were raised separately for Katie to accept or decline rather than applied.

*Brand swept.* 37 further all-caps "KYVKY" converted to **KYvKY** across `src/`, `scripts/`, and living docs (code comments, accuracy-audit output, the design-system reference page, the bill-watch reference set). Deliberately untouched: the `KYVKY_POSTAL_ADDRESS` constant, which is an identifier rather than prose, and the historical entries in `decisions.md` and `TASKS.md`, which are append-only records; the sentences in this file that quote the all-caps string as the defect being described are quotations and stay verbatim. One self-inflicted bug caught in review: the first sweep rewrote the guide's own quotation of the bad string, turning "still rendered all-caps KYVKY" into a sentence that said nothing. Restored, and the sweep re-run with the constant guarded.

**Fourth pass: copy edits accepted, `/about` warmed further.** Katie accepted all five word-choice suggestions raised in review, so they were applied to the email note, `/about`, the guide's quoted spec, and the design-system specimen together: contractions throughout ("I'm," "it's," "I'd"), `difficult to use and understand` → `hard to use` (the next paragraph proves the point, so the doubling was padding), `a confusing and outdated interface that required a subject-matter understanding of` → `an outdated interface that assumed I already understood` (shorter and more concrete), `a way to understand my state's legislation as easily as it was to follow friends on a feed` → `following my state's legislation to be as easy as following friends on a feed` (the following/following parallel is what makes the analogy work), and `In my personal life` cut as redundant.

The contraction rule is now recorded in the guide as specific to this register: the rest of the product keeps its uncontracted reference voice, and this block does not, because it is a person talking.

She then asked for `/about` to go further than mirroring the email, so **Who builds it** gained an opening "Hi," the line "KYvKY is a one-person project, and I'm the person," "I keep it running" in place of "I maintain it," a closing beat on the origin story ("So I built the thing I was looking for"), and a sign-off ("It comes straight to me, and I'd like to know. Thanks for being here."). The register on that one card is now warmer than the email note. That is intentional: the email is a one-time transactional send where brevity is a courtesy, while `/about` is a page someone chose to open. Everything else on `/about` keeps the reference voice, and every factual claim in the section is unchanged from the verified set.

## 2026-08-22 — Email branding: shared header, and two dark-mode defects found in the process

Katie asked for branding on all emails, especially the welcome email, which had none. The digest already carried a logo; the welcome email opened cold on a text heading.

**Shared module rather than a second copy.** `src/lib/email/brand.tsx` now owns `EmailBrandHeader`, `emailLogoSrc(origin)`, and `EMAIL_DARK_MODE_CSS`. Both templates render the same header, and `run-bill-digest-cron.tsx` builds its logo URL through the helper instead of hardcoding the path a second time. A third template should import, not copy.

**PNG, not the newer SVGs.** `/branding/Logo-03.png` is the only logo asset email can use. `logo-white.svg` and `logo-wordmark-white.svg` are newer and would suit dark backgrounds, but Gmail strips SVG, so an SVG logo means no logo for most recipients.

**White plate behind the mark.** The wordmark is blue artwork on transparency and there is no dark-mode variant in an email-safe format. Swapping assets by media query does not work in Gmail either. So the logo sits on an explicit white background that stays white in both themes: a quiet card in light mode, and the thing that keeps the brand legible in dark. This also changes the digest's header, which previously placed the mark directly on the body.

**Two real defects surfaced by actually rendering the result.** Both were caught by looking at the output in a dark-mode browser rather than by reading the diff.

1. **`<Body style={...} className="...">` splits across two elements.** react-email puts the class on `<body>` and the inline background on an inner `<td>`. The dark override was therefore repainting an invisible layer while the visible surface stayed `#f8fafc`, and the text overrides turned it into light-on-light. **This was already live in the digest email** — every recipient reading it in a dark-mode client has been getting washed-out text. Fixed in both prefixes by extending the selector to `.kv-bg > table > tbody > tr > td`. The extra limb is load-bearing and must not be tidied away.
2. **React escapes `>` inside `<style>`.** The fix above initially did nothing because the child combinators rendered as `&gt;`, which invalidates the whole selector list — including the valid `.kv-bg` grouped with them, so the first attempt was *worse* than no fix. Both templates now emit their CSS with `<style dangerouslySetInnerHTML={{ __html: ... }} />`.

**A third, smaller miss:** the founder-note heading shipped without a `kv-` class and was invisible in dark mode. Found by screenshot, then confirmed by an audit script over every style constant that hardcodes a colour. That audit is the check worth repeating whenever copy is added to a template.

The welcome email had no dark-mode support at all before this; it now themes background, headings, body, muted text, links, and card borders. Verified in both schemes at the rendered-HTML level, plus the plain-text fallback, which carries the logo's alt text rather than a broken image reference.

**Follow-up resolved: one prefix.** The two dark-mode rule sets are now one. All 26 `dg-` class sites in the digest became `kv-`, the base rules come from `EMAIL_DARK_MODE_CSS` in `brand.tsx`, and the five progress-meter tokens (`kv-seg`, `kv-seg-done`, `kv-done-text`, `kv-track`, `kv-seg-veto`) stay in the digest file — nothing else renders a meter — and are concatenated onto the base. A template now has exactly one prefix to reason about, and a new one gets the whole base set by importing the constant. `kv-body` is defined but unused in the digest, which is harmless: it uses `kv-ink` for body text.

Verification for this pass, since the digest is a live send and the previous two defects were both invisible in the diff:
- **Static:** every `kv-` class referenced in the rendered markup has a matching rule (scripted comparison of markup classes against the emitted stylesheet, run on both emails); zero `dg-` left in the output; zero escaped `&gt;` combinators and four verbatim ones.
- **Source:** an audit over all 12 digest style constants that hardcode a colour confirmed every usage site carries a `kv-` class. This is the check that would have caught the invisible founder-note heading, and it is the one worth repeating whenever copy is added.
- **Rendered:** a fixture exercising every themed path — a completed bill with a green meter and specific milestone, a vetoed bill with the red terminal segment, a topic-matched bill with an early meter and topic links, a committee block, the overflow line with topic links, and the full footer — screenshotted in light and dark. Light is unchanged from before the refactor; dark is correct for the first time.
- **Computed:** asserted in-browser that the visible `<td>` surface resolves to `#0f172a`, that ink/body/muted/link resolve to their dark values, that the logo plate stays `#ffffff`, and that the founder-note heading is no longer painted with the light-mode ink.
- **Build:** `next build` clean. A first run failed on `Cannot find module for page: /_error` after successfully compiling and generating all 544 pages; this was a stale `.next` from an earlier branch (the same staleness behind the phantom `src/app/browse` type errors seen all session). `rm -rf .next` and rebuild passes, and `tsc --noEmit` is now clean with no residual validator noise.

## 2026-08-24 — LegiScan quota instrumented by operation and caller

`legiscan_query_counter` has only ever bucketed by month, so a month's usage was a single integer. That is what made the June 2026 diagnosis expensive: 29,164 queries (97% of the free cap) with no breakdown, and the actual cause — the bill-detail read path calling `getBill` on every render ([§ 2026-06-26](#2026-06-26--bill-detail-rendered-entirely-from-the-db-no-live-legiscan-on-the-read-path)) — had to be reconstructed from this file rather than read off the data. Every quota question since has been answered by modelling. This makes the next one answerable by looking.

**Three buckets per call, not one.** Each billable request now writes `YYYY-MM`, `YYYY-MM:op`, and `YYYY-MM:op@caller`. The month total stays its own bucket rather than becoming a sum over the breakdown: the quota guard, the admin card and the Slack alert all read it, months recorded before today have nothing else, and a summed total would drift the moment an op slipped through untagged. `summarizeLegiscanMonthUsage` reports that gap explicitly as `unattributed` instead of hiding it.

**`ky_increment_counter_multi` (migration 054), because three buckets should not be three round trips.** It locks the counter row once via `INSERT … ON CONFLICT DO UPDATE … RETURNING`, folds the buckets into the payload in memory, and writes it back. `ky_increment_counter` is untouched — the Anthropic cache and rate-limit counters only ever touch one bucket. Verified on a scratch Postgres 16: repeated and empty/NULL bucket keys behave, a pre-existing NULL payload seeds correctly, and 300 increments across 6 concurrent clients land as exactly 300 (the row lock is what makes read-modify-write safe here).

**Caller tagging is ambient, because the client is a process-wide singleton.** `getKyLegiScanClient()` hands the same instance to the sync pipeline, the accuracy audit and every backfill script, so the caller cannot be a constructor argument. `src/lib/legiscan-caller.ts` uses `AsyncLocalStorage`: `withLegiscanCaller(tag, fn)` scopes a tag to one call tree, and `LEGISCAN_CALLER` sets one for a whole process. Tagged so far: `sync-bills`, `sync-legislators`, `sync-legislator-bios`, `sync-votes` (thin wrappers around the four pipeline functions that reach LegiScan), `accuracy-audit`, `dataset-sync`. Anything else records as `untagged`, which is a finding, not a failure — it names a call path nobody has attributed yet.

**Tags are a permanent vocabulary.** Every distinct tag is a forever key in that JSON payload, so both the op and the caller are normalized and length-capped, and `:`/`@` are stripped so a bucket key can always be parsed back apart. Adding a tag per ad-hoc script would be a slow leak; the env var exists so a one-off run can borrow an existing name.
...
## 2026-09-14 — The "agenda lost on write" committee failures were a false positive: an unpaginated audit read

**Symptom.** Every weekly accuracy run reports ~100+ committee `fail`s, all one cause: `agenda_content_hash records agenda text but no ky_committee_agenda_items rows are stored (agenda lost on write)`. The count has climbed monotonically — 79 (08-09) → 84 → 103 → 112 → 109 → 119 → 111 (09-13) — and each run's flagged meetings appear to "repair themselves" by the next day, so a point-in-time SQL query for the bad state returns 0. This is the third distinct write-timestamp/point-in-time trap in this routine (cf. the 08-11 and 08-30 entries): "0 rows in the bad state right now" kept looking like the backlog had self-healed.

**Root cause — the checker, not the data.** `checkStoredCorpusInvariants` in `src/lib/accuracy-audit/checkers/committees.ts` read `ky_committee_agenda_items` (and the meetings / materials / committees corpus tables) with a **single unpaginated PostgREST select**. PostgREST caps a response at ~1,000 rows; the agenda table holds **~2,637 rows across 224 meetings**. So the checker only ever saw the first 1,000 rows, built its `byMeeting` map from that page, and flagged **every meeting whose rows sat past the cap** as having zero agenda rows. Because `agendaContentHash` is non-empty iff `deriveAgendaItems` is non-empty (both map over `meeting.agendaItems`), those meetings all satisfied the "non-empty hash + zero rows" invariant and failed. The `sort_order`-gap warns came from the same page boundary slicing a meeting's rows in half.

**Proof it was a false positive.** Two flagged historical meetings — Capital Projects and Bond Oversight Committee 2025-01-22 (56 rows) and 2025-04-28 (65 rows) — have agenda rows whose `created_at` is 2026-07-15 and were never modified since (min = max). They were fully populated at the 09-13 audit, yet were flagged. The count grows because the corpus grows: more agenda rows past the 1,000 cap ⇒ more meetings appear empty. The "next-day repair" was an illusion — nothing was ever lost; a properly-scoped query always sees all rows.

**Why migration 050 didn't move the number.** 050 was a real fix to a real (if rarely-firing) sync-side race, and it is applied in production (`ky_replace_committee_agenda_items` exists, `prosecdef=true`) and working (agenda rows are actively (re)inserted). But the failures it was expected to clear were never coming from the sync write path — they were coming from the checker's read path, which 050 does not touch. The 08-11 "open verification gap" (did 050 apply?) is hereby closed: it applied, and it was orthogonal to this symptom.

**Fix (this change).** Added a `fetchAllRows` pagination helper (1,000-row pages with a stable `.order()`, matching `sampling.ts` and the `sitemap-data.ts:25` precedent) and routed all corpus-wide reads in the committees checker through it: `ky_committee_agenda_items`, the two `ky_committee_meetings` scans, `ky_committee_materials`, and the near-duplicate `ky_committees` scan. The agenda read was the load-bearing one; the others were latent (each under 1,000 today, but the same silent-truncation bug as the corpus grows). No production data was wrong; no data migration or repair pass is needed. Expected effect: the ~111 committee `fail`s and the `sort_order`-gap warns drop to whatever genuine committee drift remains (expected ~0).

**Rule this is another instance of:** a corpus-wide invariant that reads its inputs with an unpaginated client silently degrades into "invariant holds for the first 1,000 rows" — and reports the truncation as content failures rather than a coverage gap. Any audit read over a table that can exceed the PostgREST cap must paginate; the coverage-floor machinery does not catch this because the checker still "succeeds" with a high `checked` count.

---

## 2026-09-16 — Find my legislators: lookup first, map second, and a signup prompt on the result

**Trigger.** The 2026-09-15 PostHog review: `/members/map` is the top traffic destination, Week-1 retention is 0–3%, and one registration in the surge window. Two levers on the same page: make the lookup faster, and give a visitor who just found their legislators a reason and a path to sign up.

**Findings.**

- The lookup did not need the map, but waited for it. `DistrictMapExplorer` imported `mapbox-gl` at module scope, so the island's `next/dynamic` chunk carried the ~450KB library; the boundary and roster fetches only started once that chunk had parsed and hydrated. The point-in-polygon step itself is turf on two GeoJSON files already in `public/geo/` — pure client work, no map required.
- `/geo/*.geojson` shipped with Vercel's static default (`max-age=0, must-revalidate`): a 304 round trip per file on every return visit, on the audience we most want returning.
- ZIP lookups went through `/api/geo/zip` → Nominatim, whose usage policy caps a deployment at one request per second. Fine at 140 WAU; not at a surge. The Next data-cache `revalidate` helped repeat ZIPs on one server instance only.
- `trackDistrictMapLookup` accepted `houseDistrict`/`senateDistrict` and the only call site never passed them, and address and map-click lookups fired nothing. The "which districts search themselves" read the helper's own comment promises was never possible.
- No signup prompt existed on the page or on member profiles. The nav's "Sign up" is the only path, and the returning-user hero only speaks to people already registered.

**Decisions.**

- **Split the canvas, keep the state.** `DistrictMapCanvas` holds the `<MapGL>` tree and every Mapbox import (including the CSS); `DistrictMapExplorer` keeps all state, handlers, and the lookup, and loads the canvas with `next/dynamic` (`ssr: false`). The explorer is now server-renderable, so the island no longer needs `ssr: false`. Readiness is two flags: `dataReady` (boundaries loaded; the form, ZIP search and `?address=` auto-search key off this) and `mapReady` (`dataReady` + token; gates the canvas). A camera move requested before the canvas mounts parks in `pendingCameraRef` and is applied in `onLoad`; with nothing pending, `onLoad` fits Kentucky.
- **Preload from the head.** `react-dom`'s `preload()` for the two boundary files and the roster, with `crossOrigin: 'anonymous'` so the preloaded responses match `fetch()`'s default credentials mode and are actually reused; `preconnect()` to api.mapbox.com. The URLs move to `district-map-sources.ts` so the page, the explorer and the canvas share one definition.
- **Cache the boundaries for a day.** `next.config.ts` header on `/geo/:path*.geojson`: `max-age=86400, stale-while-revalidate=604800`. The files change only through `geo:ky-districts`, which § 2026-07-03 already gates on a parity check; a mid-day regeneration would need a URL bump, noted in the config comment.
- **ZIP geocoding in the browser first.** New `mapboxGeocodeZip` (`types=postcode`, Kentucky bbox, rejects a fuzzy match on a different postcode). Falls back to `/api/geo/zip` when there is no token or Mapbox returns nothing in the bbox, so out-of-state ZIPs still get the "couldn't match a Kentucky district" state. The route's response now carries `s-maxage=604800` so the CDN absorbs repeat ZIPs.
- **One prompt component, two surfaces.** `SignupCta` renders nothing while `useUser()` is loading or when signed in (no flash for members). Copy follows the voice guide: auth verbs "Sign up" / "Log in", body says what an account does (follow bills and committees, email digest) and nothing it does not; the footnote reuses `/about`'s free / non-partisan / never sells data claims. On the map it appears only after a lookup resolved at least one district, below the member cards; on the profile it sits above Sponsored bills, the section whose follow buttons need an account. Both pass `next=`: `/auth/register` now honors it through `safeAuthRedirectPath` (default still `/bills`), and the map restores a lookup from `?address=` or, for map clicks, `?lat=&lng=`.
- **Events for the funnel.** `district_map_lookup` fires on every resolve with `lookup_type` (`zip` | `address` | `map_click`), `zip` (null off the ZIP path), `matched`, and the district numbers. New `signup_cta_clicked` with `surface`, `auth_action`, `member_id`. The read to build: `district_map_lookup` → `signup_cta_clicked` → `user_registered`, filtered per the Tier 1 ground rules.

**Deliberately not done.** No member-follow feature (the prompt promises only bill and committee follows, which exist); no change to link order or style on profiles (blocked on Tier 1 Task 2's data); no GeoJSON re-simplification (the § 2026-07-03 parity gate makes that its own task); no Lighthouse numbers, since this environment has no Mapbox token — measure on the preview deploy and record here.
